...
This writes flashable artifacts to `firmware/export-ha-voice-pe`. The `ha_voice_pe` profile targets the Home Assistant Voice Preview Edition ESP32-S3 pin map for microphone input and the center/mute controls. It brings up the onboard Voice Kit/XMOS device over I2C before enabling the secondary I2S microphone stream. It is intentionally headless: display, touchscreen, and SD media storage report unavailable. Local speaker/TTS output is also disabled until the AIC3204/XMOS audio output path is added.
