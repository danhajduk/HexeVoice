...
void ensure_pairing_nonce() {
  ...
  ensure_crypto_ready();
  copy_cstr(g_ble.crypto_session_id, sizeof(g_ble.crypto_session_id), g_ble.onboarding_session_id);
  ESP_LOGI(
      kTag,
      "Recovery BLE identity generated session=%s nonce_prefix=%.8s key_prefix=%.8s host_bound=%d",
...
bool handle_local_recovery_write(cJSON *root) {
  ...
  if (std::strcmp(pairing_nonce, g_ble.pairing_nonce) != 0) {
    ESP_LOGW(
        kTag,
        "Recovery BLE local credentials nonce mismatch session=%s payload_nonce_prefix=%.8s current_nonce_prefix=%.8s crypto_session=%s",
...
bool validate_envelope_binding(cJSON *root, EnvelopeFields *fields) {
  ...
  if (std::strcmp(fields->pairing_nonce, g_ble.pairing_nonce) != 0) {
    ESP_LOGW(
        kTag,
        "Recovery BLE encrypted credentials nonce mismatch session=%s payload_nonce_prefix=%.8s current_nonce_prefix=%.8s crypto_session=%s",
