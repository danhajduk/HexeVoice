...
- Firmware now has an experimental on-device Stop keyword provider hook. The per-frame audio path calls `inspect_playback_stop_word_frame(...)`; a future detection can call `stop_playback("voice_stop")` during playback or `cancel_active_session("voice_stop")` during an active voice session. Current profiles still report `missing_micro_wake_word_inference_engine` or `missing_stop_keyword_model` because no trained Stop keyword model is linked yet, so backend STT interrupt remains the active fallback.
