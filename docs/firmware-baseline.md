...
- Firmware now has an experimental on-device Stop keyword provider hook configured from Kevin Ahrendt's microWakeWord Stop release (`stop.json`/`stop.tflite`, cutoff 0.50, sliding window 5, tensor arena 21000). The Stop JSON/TFLite assets are checked in under `firmware/main/voice/models/`, the TFLite asset is embedded into the firmware image, and the per-frame audio path calls `inspect_playback_stop_word_frame(...)`; a future detection can call `stop_playback("voice_stop")` during playback or `cancel_active_session("voice_stop")` during an active voice session. Backend STT interrupt remains the active fallback until local preprocessing/inference is activated.
- Firmware now has an experimental endpoint microWakeWord provider hook configured for the official ESPHome `alexa` v2 model, treating the spoken wake word `Alexa` as the initial Hexe-compatible local wake path. The Alexa JSON/TFLite assets are checked in under `firmware/main/voice/models/`, the TFLite asset is embedded into the firmware image, and heartbeat capabilities expose the model manifest/TFLite source, version, hashes, embedded byte size, cutoff, sliding window, feature step, tensor-arena size, and `micro_wake_engine` diagnostics.
- Firmware links Espressif's `esp-tflite-micro` runtime and ESP-NN acceleration through `voice/micro_wake_engine.{h,cpp}`. The adapter initializes from embedded model assets and validates TFLite headers/schema, but currently reports the local providers unavailable because the microWakeWord feature-preprocessing bridge is still pending.
