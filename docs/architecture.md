...
The WebSocket is implemented by `src/hexevoice/voice/session_manager.py`. It supports one connected endpoint and one active session for the MVP, accepts `session.start`, `audio.chunk`, `audio.end`, `session.cancel`, `session.ping`, `command.ack`, `command.error`, and endpoint TTS playback acknowledgement events through the versioned `VoiceEventEnvelope`, and returns `wake.accepted`, `session.state`, `session.completed`, `session.cancelled`, or `session.error` events through the same envelope. The envelope contract and JSON schema/examples live in `docs/voice-event-envelope.md` and `docs/voice-event-envelope/`.
