...
- `GET /api/voice/audio/ws?endpoint_id=<endpoint_id>&encoding=pcm_s16le&sample_rate_hz=16000&channels=1` is the live binary PCM audio channel. It requires the endpoint control WebSocket to already be connected and an active voice session before audio frames are accepted.
