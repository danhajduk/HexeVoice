...
The status route returns an explicit `state_projection` with `connection_state`, `ux_state`, `session_state`, and `transport_health`, plus the active session snapshot, recent transcript/response/TTS/error metadata, structured command acknowledgements/errors, event diagnostics, and supported operator actions. The frontend voice endpoint dashboard consumes this route instead of placeholder cards.
