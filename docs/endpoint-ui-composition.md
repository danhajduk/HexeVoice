...
The optional `weather_image` component uses the same status vocabulary as other weather components. A ready or stale background must provide `image_url`, `revision`, `sha256`, `content_type`, `width`, and `height`; image bytes remain outside the shared MQTT event. `bg` remains accepted as a compatibility alias. Weather backgrounds receive a 1.15x centered zoom before the final 800x420 crop; radar frames retain their original map scale.
