...
The retired XVF3800 plus XIAO ESP32-S3 target is no longer a supported firmware
profile.
...
- `audio.input.dsp.vad` is true for the HA Voice PE hardware audio frontend.
  Firmware VAD is separately available on all four profiles and active today on
  the two active firmware profiles.
- The three display profiles expose `display.size_inches`, `display.width_px`,
  and `display.height_px`; `ha_voice_pe` intentionally keeps them `null`.
