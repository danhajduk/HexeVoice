...
- Starting a batch opens a short endpoint-scoped Speaker ID enrollment capture window. Completed phrases are transcribed and stored as capture candidates, but assistant routing and TTS playback are suppressed so the endpoint does not answer each enrollment phrase with the normal assistant fallback.
