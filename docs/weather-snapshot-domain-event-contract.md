...
Interaction may include a `data.weather_image` component for its prepared weather background. The event carries only the reference metadata: `status`, `revision`, `image_url`, `sha256`, `content_type`, `width`, and `height`. URLs must be public HTTP(S) references without embedded credentials or sensitive query parameters. HexeVoice also accepts `data.bg` as a compatibility alias.

HexeVoice selects a visual source in this order: ready or stale `radar`, ready or stale `weather_image`, then a locally generated blank background. It verifies the selected source checksum and center-crops it to the endpoint's 800x420 RGB888 content asset. The current Interaction weather backgrounds are 1672x941 and require no source-side resizing.
