...
runtime/firmware/hexe_firmware_waveshare_s3_touch_lcd_1_85c_box_v2.bin
```

`hexe_firmware.bin` is the default PE OTA artifact. Profile-specific OTA pushes
can target `hexe_firmware_ha_voice_pe.bin` or
`hexe_firmware_waveshare_s3_touch_lcd_1_85c_box_v2.bin`. The retired
`esp_box_3` artifact name is recognized only for legacy installations. Runtime
firmware binaries and generated manifests are intentionally ignored by git;
`runtime/firmware/.gitkeep` keeps the directory present.
